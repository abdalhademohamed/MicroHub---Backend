...
export class SharableOfferController {
  ...
  async update(
    ...
    return await this.sharableOfferService.update(
      sharableOfferId,
  ...
  async create(
    @Request() req: any,
    @Body() createSharableOfferDto: CreateSharableOfferDto,
  ): Promise<SharableOfferEntity> {
    ...
    //   throw new BadRequestException("User not authenticated");
    // }
    return this.sharableOfferService.createSharableOffer(
      createSharableOfferDto,
    );
  }

  @UseGuards(AccessTokenGuard, RolesGuard) // Ensure AccessTokenGuard is first
  @Roles(Role.SUPERADMIN, Role.ACCOUNTANT, Role.COORDINATOR, Role.RECEPTIONIST)
