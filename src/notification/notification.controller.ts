...
export class NotificationController {
  ...
  async saveToken(@Body() body: { token: string }, @Request() req: any) {
    const userId = req.user.sub; // Extract user ID from the request object
    const { token } = body;
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        ...
        errors: [{ token: "token1", error: "Error message" }],
      },
    },
  })
  async sendNotificationToMultipleUsers(
    ...
    if (!userIds || userIds.length === 0 || !title || !message) {
      throw new HttpException(
        "User IDs, title, and message are required",
        HttpStatus.BAD_REQUEST,
      );
    }

    try {
      return await this.notificationService.sendNotificationToMultipleUsers(
        userIds,
        title,
        message,
      );
